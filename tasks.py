...
def clean_scores(c):
    """
    Remove split files generated by run_scores from discovery folder.
    """
    import os
    import glob

    discovery_dir = c.config.get("output_discovery", "output_data/Discovery")
    targets = glob.glob(os.path.join(discovery_dir, "split_net*.tsv")) + [
        os.path.join(discovery_dir, "table_split.csv")
    ]

    if not any(os.path.exists(t) for t in targets):
        print("🫧 No score artifacts found to clean.")
        return
    ...
    print("🧹 Conformal score outputs cleaned.")
